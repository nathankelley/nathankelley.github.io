const list = [
    {
        name: "Bechler Falls", 
        img: "./img/falls.jpg",
        alt: "Image of Bechler Falls",
        distance: "3 miles", 
        difficulty: "Easy",
        description: 
        "Beautiful short hike along the Bechler river to Bechler Falls",
        directions:
        "Take Highway 20 north to Ashton. Turn right into the town and continue through. Follow that road for a few miles then turn left again onto the Cave Falls road.Drive to the end of the Cave Falls road. There is a parking area at the trailhead.",
    },
    {
        name: "Teton Canyon",
        img: "./img/falls.jpg",
        alt: "Image of Bechler Falls",
        distance: "3 miles",
        difficulty: "Easy",
        description: "Beautiful short (or long) hike through Teton Canyon.",
        directions:
        "Take Highway 33 East to Driggs. Turn left onto Teton Canyon Road. Follow that road for a few miles then turn right onto Stateline Road for a short distance, then left onto Alta Road. Veer right after Alta back onto Teton Canyon Road. There is a parking area at the trailhead",
    },
    {
        name: "Denanda Falls", 
        img: "./img/falls.jpg",
        alt: "Image of Bechler Falls",
        distance: "7 miles", 
        difficulty: "Moderate",
        description: "Beautiful short (or long) hike through Teton Canyon.",
        directions:
        "Take Highway 20 north to Ashton. Turn right into the town and continue through. Follow that road for a few miles then turn left again onto the Cave Falls road.Drive to the end of the Cave Falls road. There is a parking area at the trailhead.",
    }
];

// Hike Model
export default class HikeModel {
    constructor(name, img, alt, distance, difficulty, description, directions) {
      // We need a constructor...but in this case it isn't doing much
      name = this.name;
      img = this.img;
      alt = this.alt;
      distance = this.distance;
      difficulty = this.difficulty;
      description = this.description;
      directions = this.directions;
    }


    getAllHikes() {
      // should return a list of all the hikes.
      
    }
    getHikeByName(hikeName) {
      // filter the hikes for the record identified by hikeName and return it
      return ;
    }
}

function addList(){
    const theList = document.getElementById("hikes");

    for  (l in list){
        theList.innerHTML += "<li>" + list[l].name + "<img src= " + list[l].img + ">" + list[l].distance + "<br>" + list[l].difficulty + "</li>";
    }
    
}

// addList();